...
# 4. dockerfile 생성 및 실행 
```
# 해당 URL 참고 : https://docs.aws.amazon.com/ko_kr/lambda/latest/dg/python-image.html#python-image-instructions
dockerfile 작성
docker build --platform linux/amd64 -t [폴더명]:latest .
docker run --platform linux/amd64 -p 9000:8080 [폴더명]:latest

curl 실행 (window)
Invoke-WebRequest -Uri "http://localhost:9000/2015-03-31/functions/function/invocations" -Method Post -Body '{"model": "anthropic.claude-3-sonnet-20240229-v1:0", "prompt": "영어 과목 중 요약문 완성 풀기에 대한 설명을 300자 이내 한글로 생성하고, 요악문 완성 관련 문제를 영어 지문1개 당 영어 문제 1개 씩 5지선다로 3 문제 생성 및 답을 생성해줘.","useDefaultJsonFormat": true}' -ContentType "application/json"
```